...
class FileProcessing():
    ...
    def write_DISCON(self, turbine, controller, param_file='DISCON.IN', txt_filename='Cp_Ct_Cq.txt'):
        ...
        file.write('{:<11d}         ! PC_LUT_N          - Number of values in minimum blade pitch lookup table (should equal number of values in PS_WindSpeeds and PS_BldPitchMin)\n'.format(len(controller.APC_R)))
        file.write('{}              ! PC_PwrRating      - Wind speeds corresponding to minimum blade pitch angles [m/s]\n'.format(''.join('{:<4.8f} '.format(controller.APC_R[i]) for i in range(len(controller.APC_R)))))
        file.write('{}              ! PC_BldPitchMin    - Minimum blade pitch angles [rad]\n'.format(''.join('{:<10.8f} '.format(controller.APC_B[i]) for i in range(len(controller.APC_R)))))
        file.write('\n')
        file.write('!------- SHUTDOWN -----------------------------------------------------------\n')
